...
class Enc_only(nn.Module):
    ...
    def __init__(
        ...
        self.action_emb = nn.Embedding(action_seq_len, embed_dim)
    ...
    def forward(
        ...
        input_seq = torch.cat([state_x, action_seq])

        # encode the state, goal and latent z into the hidden dim
        encoder_output = self.encoder(input_seq)

        pred_actions = self.action_pred(encoder_output[:, 1:, :])
...
class EncDec(nn.Module):
    ...
    def forward(
        ...
        # encode the state, goal and latent z into the hidden dim
        encoder_output = self.encoder(state_x)
